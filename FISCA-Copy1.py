...
def pagina_dashboard_executivo(dados, filtros):
    ...
    # ========== KPIs PRINCIPAIS ==========
    st.markdown("<div class='sub-header'>📈 Indicadores Gerais</div>", unsafe_allow_html=True)
    ...
    with col1:
        ...
        st.metric("📋 Total Infrações", f"{int(total_fisc):,}")
    ...
    with col2:
        total_encer = df_dash['qtd_encerramentos'].sum()
        st.metric("✅ Encerramentos", f"{int(total_encer):,}")
    
    with col3:
        ciclos_completos = df_dash['qtd_ciclos_completos'].sum()
        st.metric("🔄 Ciclos Completos", f"{int(ciclos_completos):,}")
    ...
    # ========== TAXA DE CONVERSÃO E PRODUTIVIDADE ==========
    col1, col2 = st.columns(2)
    
    with col1:
        # Taxa de conversão por ano
        df_dash['taxa_conversao_calc'] = (df_dash['qtd_nfs_emitidas'] / df_dash['qtd_infracoes_lavradas'] * 100).round(2)
        
        fig = px.line(
            df_dash,
            x='ano',
            y='taxa_conversao_calc',
            title='Taxa de Conversão (Infração → NF)',
            template=filtros['tema'],
            markers=True
        )
        
        fig.add_hline(y=70, line_dash="dash", line_color="green", annotation_text="Meta: 70%")
        fig.update_layout(height=400, yaxis_title='Taxa (%)')
...
def pagina_tipos_infracoes(dados, filtros):
    ...
    # Agregar por tipo
    df_agregado = df_rank.groupby(['codigo_infracao', 'descricao_infracao', 'tipo_infracao']).agg({
        'qtd_ocorrencias': 'sum',
        'qtd_empresas': 'sum',
        'valor_total': 'sum',
        'valor_medio': 'mean'
    ...
    with col1:
        ...
        fig = px.bar(
            ...
            x='qtd_ocorrencias',
            orientation='h',
            title='Top 30 - Por Volume',
            template=filtros['tema'],
            color='qtd_ocorrencias',
            color_continuous_scale='Reds',
            hover_data=['descricao_infracao']
        )
    ...
    with col2:
        df_top_valor = df_agregado.nlargest(30, 'valor_total')
    ...
    # ========== TABELA COMPLETA ==========
    st.markdown("<div class='sub-header'>📋 Tabela Completa</div>", unsafe_allow_html=True)
    
    df_display = df_agregado.sort_values('qtd_ocorrencias', ascending=False).copy()
    df_display['ranking'] = range(1, len(df_display) + 1)
    
    cols = ['ranking', 'codigo_infracao', 'descricao_infracao', 'tipo_infracao', 
            'qtd_ocorrencias', 'qtd_empresas', 'valor_total', 'valor_medio']
